...
let projectFile = "./src/Thoth.Elmish.Debouncer.fsproj"
let demoFile = "./demo/Demo.fsproj"
...
Target.description "Start a standalone version of the demo, used to write/debug it"
let watchDemo = BuildTask.create "WatchDemo" [ yarnInstall; dotnetRestore ] {
...
let copyDemoFiles = BuildTask.create "CopyDemoFiles" [ buildDemo ] {
    Directory.create "docs_deploy/demo"

    !! "demo/output/*"
    |> Shell.copyFiles "docs_deploy/demo"
}
...
let buildDocs = BuildTask.create "BuildDocs" [ copyDemoFiles ] {
    Yarn.exec "nacara" id
}

Target.description "Start a local webserver allowing you to work on the docs site locally"
let _watchDocs = BuildTask.create "WatchDocs" [ copyDemoFiles ] {
